...
## ND: If you forget to create the fabric folder then set GOPATH to $HOME and don't forget to remove that fabric folder allthrough the rest of the installation
...
## ND: The code that worked to install basic chaincode is given below:
...
## ND: You don't need to use the multiplexer if you are using H to login. You can use multiple ssh sessions to monitor to logs
## 
tmux control
```bash
...
## ND: 'tmux new-window' opens up a new window/pannel while the 'tmux split-window -d' splits the session 
...
# detach from session
CTRL + b + d
# ND: attach to session using 'tmux attach'
...
source ./org1.sh
## ND: You can create other env files like org2.sh to set environment variable for other organisations
## ND Note that you have to call this script to set the env variable each time you login as the env variables do not persist logins on the cloud
