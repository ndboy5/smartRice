...
if [ "$CC_SRC_PATH" = "NA" ]; then
	...
	if [ "$CC_NAME" = "basic" ]; then
		...
		echo $'\e[0;32m'user$'\e[0m' chaincode
		CC_SRC_PATH="../chaincode/user"
	elif [ "$CC_NAME" = "product" ]; then
		echo $'\e[0;32m'product$'\e[0m' chaincode
		CC_SRC_PATH="../chaincode/product"
	elif [ "$CC_NAME" = "order" ]; then
		echo $'\e[0;32m'order$'\e[0m' chaincode
		CC_SRC_PATH="../chaincode/order"
	else
