...
const Schema = require('./publisher.json');
...
class Publisher extends Base {
    ...
    async publishData(ctx,id, data) {
       const asset = JSON.parse(data);
       asset.id = id;
       let currentTime = new Date();
       asset.createdAt = currentTime.toString();
       asset.recordsAccess = {}; // map ipfs hash to patient email,name,document name
       const buffer = Buffer.from(JSON.stringify(asset));
       await ctx.stub.putState(asset.id, buffer);
       return asset; 
    }

    async giveAccess(ctx, dataId, requestId) {
     const asset = await this.getData(ctx, dataId);
     if (!asset) {
         throw new Error(`The asset record ${dataId} does not exist`);
     }
     if(!asset.recordsAccess[dataId]){
      asset.recordsAccess[dataId] = [];
     }
     asset.recordsAccess[dataId].push(requestId);
     const buffer = Buffer.from(JSON.stringify(asset));
